...
# Every single run must include these to be valid.
...
# Only these keys are allowed in log_metrics() for the respective run type.
ALLOWED_METRICS: Dict[RunType, Set[str]] = {
    RunType.RETRIEVAL: {
        ...
        "retrieval_latency", # Implicitly allowed for performance tracking
    },
    RunType.EVAL: {
        "answer_accuracy",
        "citation_precision",
        ...
        "avg_confidence",
        "refusal_rate",
        "llm_latency",      # Implicitly allowed
    },
    RunType.GUARDRAIL: {
        "refusal_triggered",       # 0 or 1
        "confidence_score",
        "num_supported_sentences",
        "num_total_sentences",
        "alignment_score",         # Often calculated with confidence
        "recall_score",            # Often calculated with confidence
        "retrieval_latency",
    }
}

# 3. Context-Specific Params
# Non-metric data allowed for specific run types (e.g. refusal reasons).
...
def validate_run_structure(tags: Dict[str, Any], metrics: Dict[str, float]) -> None:
    ...
    Args:
        tags: Dictionary of tags/params logged to MLflow.
        metrics: Dictionary of numeric metrics logged to MLflow.
    ...
    # Strict Mode: Fail on unknown metrics
    if unknown_metrics:
        raise SchemaViolationError(
